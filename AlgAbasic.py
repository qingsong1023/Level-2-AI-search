...
# GA algorithm implementation for TSP
# Reserved variables for parameters in the added note
max_it = 800
pop_size = 50

# Generating the initial population
def generate_initial_population(pop_size, num_cities):
    population = []  # Initialise an empty list
    # loop pop_size several times, generating a new travel route in each loop
...
# Calculate a given travelling path
def tour_length(tour):
    tour_length = 0  # initialisation
...
# The selection process is implemented using the roulette wheel selection method 
def select_parents(population, fitness_scores):
    parents = []  # initialise an empty list parents
    fitness_total_scores = sum(fitness_scores)  # calculate the sum of all fitness scores
    # iterate over each individual and use a random number to decide whether to select that individual or not
    for _ in range(len(population)):
        pick = random.uniform(0, fitness_total_scores)
        current = 0
...
# Implementing a simple OX
def crossover(parent_1, parent_2):
    start, end = sorted(random.sample(range(len(parent_1)), 2))  # random selection of crossover fragments
    child = [None]*len(parent_1)
    child[start:end] = parent_1[start:end]
    p2_elements = [item for item in parent_2 if item not in child]
    included = set(parent_1[start:end])
    p2_elements = [item for item in parent_2 if item not in included]
    p2_idx = 0
    # filling vacant positions in the children
    for i in range(len(child)):
        if child[i] is None:
            child[i] = p2_elements[p2_idx]
            p2_idx += 1
    return child
...
def genetic_algorithm(num_cities, dist_matrix, pop_size=50, max_it=800):
    ...
    for _ in range(max_it):
        ...
        for tour in population:
            ...
            if length < best_length:
                best_length = length
                best_tour = tour
            fitness_scores.append(1 / length)  # Inverting to make lower length have higher fitness        
        # Selection
...
tour, tour_length = genetic_algorithm(num_cities, dist_matrix, pop_size, max_it)
