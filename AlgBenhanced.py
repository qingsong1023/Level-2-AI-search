...
b = 5  # Heuristic importance factor
p = 0.1  # Pheromone evaporation rate
initial_pheromone = 1  # Initial pheromone level on paths
...
# Heuristic function to calculate attractiveness
def attractiveness(pheromone, distance, alpha, beta):
    return (pheromone ** alpha) * ((1.0 / distance) ** beta) if distance != 0 else 0

# Choose the next city using a probabilistic approach
def select_next_city(current_city, visited):
    probabilities = []
    city_candidates = []  # List to store unvisited cities
    # Calculate probabilities for unvisited cities
    for i in range(num_cities):
        if i not in visited:
            prob = attractiveness(pheromone_matrix[current_city][i], dist_matrix[current_city][i], a, b)
            if prob > 0:  # Ensure that the probability is positive before adding
                probabilities.append(prob)
                city_candidates.append(i)
    # Normalize probabilities and use roulette wheel selection
    if probabilities:
        total = sum(probabilities)
        probabilities = [prob / total for prob in probabilities]
        next_city = random.choices(city_candidates, weights=probabilities, k=1)[0]
        return next_city
    else:
        # Fallback: select randomly among unvisited
...
for iteration in range(max_it):
    ...
    for ant in range(num_ants):
        ...
        tour = [start_city]
        visited = set(tour)
        # Construct the tour by visiting all cities
    ...
    # Update pheromones for all paths
    for i in range(num_cities):
        for j in range(num_cities):
            pheromone_matrix[i][j] *= (1 - p)  # Evaporation
            if i != j and [i, j] in best_tour:
                pheromone_matrix[i][j] += best_tour_length  # Reinforcement
