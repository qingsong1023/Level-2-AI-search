...
def calculate_tour_length(tour, dist_matrix):
    total_length = sum(dist_matrix[tour[i]][tour[i + 1]] for i in range(len(tour) - 1))
    total_length += dist_matrix[tour[-1]][tour[0]]  # Return to the starting city
    return total_length

# The selection process is implemented using the tournament selection method
def tournament_selection(population, fitness_scores, tournament_size=5):
    selected_parents = []  # initialise an empty list parents
...
def pmx_crossover(parent_1, parent_2):
    ...
    for index in range(start, end):
        element = parent_2[index]
        if element not in child:
            pos = index
            while child[pos] is not None:
                pos = parent_2.index(parent_1[pos])
            child[pos] = element
    # Fill the remaining positions with parent 2's elements
    for i in range(size):
        if child[i] is None:
            child[i] = parent_2[i]
    return child
...
def genetic_algorithm(num_cities, dist_matrix, pop_size=50, max_it=800):
    ...
    for _ in range(max_it):
        # Evaluate
        fitness_scores = [1 / calculate_tour_length(tour, dist_matrix) for tour in population]
        # Selection
