...
# ACO algorithm implementation for TSP
# Parameters
num_ants = 10  # number of ants
max_it = 40  # maximum number of iterations
a = 1  # pheromone importance factor
b = 5  # heuristic information importance factor
p = 0.1  # pheromone evaporation rate
initial_pheromone = 1  # initial pheromone level
pheromone_intensity = 100  # increasing pheromone intensity

# Initialising the pheromone
pheromone_matrix = [[initial_pheromone for _ in range(num_cities)] for _ in range(num_cities)]

# Calculate the probability of moving from one city to another
def probability_of_selection(pheromone_level, distance, a, b):
    if distance == 0:
        return 0
    heuristic_value = 1 / distance
    return (pheromone_level ** a) * (heuristic_value ** b)

# Initialise the optimal trip
best_tour = None
...
for iteration in range(max_it):
    ...
    for ant in range(num_ants):
        tour = [random.randint(0, num_cities - 1)]  # randomly selected starting city
        current_city = tour[0]
        # Route construction
        for _ in range(1, num_cities):
            probabilities = []
            for next_city in range(num_cities):  # calculate the probability of the next city being chosen
                if next_city not in tour:
                    prob = probability_of_selection(pheromone_matrix[current_city][next_city], dist_matrix[current_city][next_city], a, b)
                    probabilities.append((prob, next_city))
            # select Next City
            probabilities = [(prob[0], prob[1]) for prob in probabilities if prob[0] > 0]
            if probabilities:
                total = sum(prob[0] for prob in probabilities)
                probabilities = [(prob[0]/total, prob[1]) for prob in probabilities]
                next_city = random.choices([prob[1] for prob in probabilities], weights=[prob[0] for prob in probabilities], k=1)[0]
            else:
    ...
    for tour in all_tours:
        ...
        pheromone_addition = pheromone_intensity / tour_length
        for i in range(num_cities - 1):
            pheromone_matrix[tour[i]][tour[i+1]] += pheromone_addition
        pheromone_matrix[tour[-1]][tour[0]] += pheromone_addition
